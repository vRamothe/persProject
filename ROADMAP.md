...
| 11 | **Rate limiting on login & quiz** | Tech | Sonnet | No throttling on `ConnexionView.post` or `soumettre_quiz`. Add `django-axes` or rate-limit decorator against brute-force/spam. |
| 12 | **Email verification on signup** | Tech | Sonnet | Anyone can register with a fake email. Add token-based verification to prevent dummy accounts. |
...
| 15 | **Technical SEO (sitemaps)** | Tech | Sonnet | `django.contrib.sitemaps` → dynamic `sitemap.xml` with all free lessons. Submit to Google Search Console. |
...
| 20 | **Difficulty levels on questions** | Feature | Sonnet | Tag questions facile/moyen/difficile. Chapter quiz guarantees balanced mix. |
| 21 | **Lesson bookmarks / notes** | Feature | Sonnet | `UserNote(user, lecon, texte, position)` model for student annotations. |
...
| # | Task | Type | LLM | Detail |
|---|------|------|-----|--------|
| 27 | **DRY quiz submission logic** | Refactor | **Opus** | `soumettre_quiz` and `soumettre_quiz_chapitre` share ~80% code. Extract `_evaluer_reponses()`. |
| 28 | **Content admin UX — bulk import** | Tool | Sonnet | CSV/JSON bulk import for questions (with tolerances, options). |
| 29 | **Admin content analytics** | Feature | Sonnet | Question success rates, lesson drop-off rates — helps improve content quality. |
| 30 | **Full-text search** | Feature | Sonnet | PostgreSQL `SearchVector` + `SearchRank`, built into Django. |
| 31 | **PDF export of lessons** | Feature | Sonnet | Offline access via `weasyprint` rendering Markdown HTML to PDF. |
| 32 | **Accessibility (a11y)** | Tech | **Opus** | `aria-label`, focus management after HTMX swaps, `<fieldset>`/`<legend>` on quiz radios. |
| 33 | **Logging & monitoring** | Infra | Sonnet | Structured logging, Sentry, `/health/` endpoint. |
| 34 | **Database backups** | Infra | Sonnet | Schedule automated Postgres backups (Heroku `pg:backups:schedule` or equivalent). |
